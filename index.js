...
tekraredensayilar = [];
sayilar.forEach(sayi => {
  const anahtar = sayi.toString();
  sayilarSayac[anahtar] = (sayilarSayac[anahtar] || 0) + 1;
});
for (let sayi in sayilarSayac) {
  if (sayilarSayac[sayi] > 1) {
    tekraredensayilar.push(`${sayi} sayısı ${sayilarSayac[sayi]} kere tekrar edilmiştir`);
  }
}
console.log(tekraredensayilar);

console.log("sayilar dizisinde", sayilar.length, "adet sayı var.");
